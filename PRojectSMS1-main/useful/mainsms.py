...
def Report():
    ...
    cur = con.cursor()
    cur.execute("select * from orders where Order_date between %s AND %s",(Date1.get(),Date2.get()))
    ...
    cur2 = con2.cursor()
    cur2.execute("select * from orders where Order_date between %s AND %s",(Date1.get(),Date2.get()))
